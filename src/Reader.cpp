...
int Reader::HowManyLines(std::string& filepath){
  ...
  if(in_file.is_open()){
    ...
    G4String ed = "Description: Failed to open file with filepath = ";
    ed += filepath;
    G4Exception("Reader::HowManyLines", "0", JustWarning, ed);
  }
...
int Reader::ReadTwoColumnsCsv(std::string & filepath, std::vector<double> & column_1, std::vector<double> & column_2, double c1_units, double c2_units){
  ...
  if(in_file.is_open()){
    ...
    G4String ed = "Description: Failed to open file with filepath = ";
    ed += filepath;
    G4Exception("Reader::ReadTwoColumnsCsv", "1", JustWarning, ed);
  }
  ...
  if(len>counter){
    ...
    G4Exception("Reader::ReadTwoColumnsCsv()", "2", JustWarning, ed);
  }
