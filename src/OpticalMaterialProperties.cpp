...
G4MaterialPropertiesTable* OpticalMaterialProperties::LAr()
{
  ...
  for (int i = 0; i < 2; i++) {
    lambda_limits[i] = 1239.675 / energies[i]; // nm. We used: lambda = 2*pi*hbar*c/E
  }
  ...
  G4double lambda_aux[dim_lambda] = {0.0};
  ...
  for (int j = 0; j < dim_lambda; j++) {
    lambda[j] = lambda_aux[j];
  }
  ...
  mpt->AddProperty("RINDEX", energies, rindex, 5);
