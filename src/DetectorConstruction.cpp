...
DetectorConstruction::DetectorConstruction():
  ...
  //num_phsensors(48)
    world_size_(10. * m),
    plate_width_(1000.0 * mm), // X 112.0
    plate_thickn_(4.0 * mm), // Y
    plate_length_(1000.0 * mm), // Z 491.5
...
void DetectorConstruction::ConstructDichroicFilter(G4VPhysicalVolume* world_phys_vol) const //vol2
{
    // DICHROIC FILTER ///////////////////////////////////////
    ...
    G4Box* filter_solid_vol =
        new G4Box(filter_name, plate_width_ / 2., filter_thickn_ / 2., plate_length_ / 2.); //width: 112 mm, thickness: 1mm, length: 491.5 mm, 

    //G4Material* filter_material = G4NistManager::Instance()->FindOrBuildMaterial("G4_SILICON_DIOXIDE");
    G4Material* filter_material = G4NistManager::Instance()->FindOrBuildMaterial("G4_lAr");
    ...
    G4VPhysicalVolume* filter_phys_vol = new G4PVPlacement(nullptr, filter_pos, // vol1 
        filter_logic_vol, filter_name,
        world_phys_vol->GetLogicalVolume(),
        false, 0, true);

    //now create the surface. It is a dichroic filter, so we need more things... DONE
    ...
    filtersurf_opsurf->SetMaterialPropertiesTable(OpticalMaterialProperties::LAr());
    //new G4LogicalSkinSurface("FILTER_SURFACE", filter_logic_vol, filtersurf_opsurf); // replace with border, takes two physical, not logical volumes
    new G4LogicalBorderSurface("FILTER_SURFACE", world_phys_vol, filter_phys_vol, filtersurf_opsurf); //physical volume of media photon passes through, in order of how photon passes through them
}
